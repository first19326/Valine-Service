...
function sendNotification(currentComment, defaultIp) {
    ...
    // 发送通知邮件
    receiveMail = process.env.RECEIVE_EMAIL;
    if (currentComment.get('mail') !== receiveMail) {
    ...
    query.get(rid).then(function (parentComment) {
        if (parentComment.get('mail') && parentComment.get('mail') !== process.env.RECEIVE_EMAIL) {
            mail.send(currentComment, parentComment);
        } else {
            console.log('被AT者匿名, 不会发送通知');
        }
        
    }, function (error) {
        console.warn('获取AT对象失败!');
    });
}
